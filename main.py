...
def global_exception_handler(exctype, value, traceback):
    ...
    error_msg = str(value)
    if exctype == TypeError and (
        "WPARAM is simple" in error_msg
        or "WNDPROC return value cannot be converted to LRESULT" in error_msg
    ):
...
def ping_host(host):
    ...
    try:
        output = subprocess.check_output(
            command, stderr=subprocess.STDOUT, universal_newlines=True, timeout=5
        )
...
def login_to_router(ip, username, password, use_api_lib=True):
    ...
    if HUAWEI_API_AVAILABLE and use_api_lib:
        try:
            ...
            return (
                client,
                None,
                f"Login Successful (API) - {device_info.get('devicename', '')} {device_info.get('HardwareVersion', '')}",
            )
        except Exception as e:
    ...
    if not reachable:
        return (
            None,
            None,
            f"Router at {ip} is not reachable. Check your connection and IP address.",
        )
    ...
    try:
        ...
        if response.status_code != 200:
            return (
                None,
                None,
                f"Failed to get CSRF token (Status: {response.status_code})",
            )
        ...
        for i, payload in enumerate(monitoring_payloads):
            ...
            response = session.post(
                f"http://{ip}{SET_MONITORING_ENDPOINT}",
                data=payload,
                headers=headers,
                timeout=10,
            )
            if response.status_code == 200:
                print(
                    f"Monitoring payload {i + 1} (Status: {response.status_code}, Response: {response.text})"
                )
            else:
                print(
                    f"Monitoring payload {i + 1} failed (Status: {response.status_code}, Response: {response.text})"
                )

        response = session.post(
            login_url, data=login_payload, headers=headers, timeout=10
        )
        if response.status_code == 200:
            info_response = session.get(
                f"http://{ip}{INFO_ENDPOINT}", headers=headers, timeout=10
            )
            if info_response.status_code == 200:
                ...
                if firmware is not None:
                    return (
                        session,
                        token,
                        f"Login Successful (Firmware: {firmware.text})",
                    )
        ...
        return (
            None,
            None,
            f"Connection to {ip} timed out. Verify the router is powered on and the IP is correct.",
        )
    except requests.exceptions.ConnectionError:
        return (
            None,
            None,
            f"Cannot connect to {ip}. Check if the router is online and accessible.",
        )
    except ET.ParseError:
        return (
            None,
            None,
            f"Received invalid XML from router. The device at {ip} may not be a Huawei router.",
        )
    except Exception as e:
...
def fetch_signal_data_api(self, client, ip):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if "CurrentNetworkType" in status_info:
                    ...
                    self.log_message(
                        f"Network type from monitoring status: {network_type_code}",
                        log_type="detailed",
                    )
                    ...
                    if network_type_code in ["21", "22", "23", "101", "102", "103"]:
                        self.log_message(
                            "Detected 5G network type, checking for 5G bands",
                            log_type="detailed",
                        )
            except Exception as e:
                self.log_message(
                    f"Error getting monitoring status: {str(e)}", log_type="detailed"
                )
            ...
            try:
                ...
                # Check for 5G bands
                if "NrBand" in net_mode_info and net_mode_info["NrBand"] not in [
                    "--",
                    "0",
                ]:
                    self.log_message(
                        f"5G NR band found: {net_mode_info['NrBand']}",
                        log_type="detailed",
                    )
                    ...
                    try:
                        ...
                        if active_5g_bands:
                            ...
                            if signal_data.get("mode") not in [
                                "21",
                                "22",
                                "23",
                                "101",
                        ...
                        self.log_message(
                            f"Error parsing 5G NrBand hex: {str(e)}",
                            log_type="detailed",
                        )
                ...
                if "LTEBand" in net_mode_info:
                    ...
                    self.log_message(
                        f"LTE band hex: {lte_band_hex}", log_type="detailed"
                    )
                    ...
                    if lte_band_hex != "--" and lte_band_hex != "0":
                        ...
                        try:
                            ...
                            if active_bands:
                                if (
                                    "bands" in signal_data
                                    and signal_data["bands"] != "--"
                                ):
                                ...
                                if (
                                    "band" not in signal_data
                                    or signal_data["band"] == "--"
                                ):
                                    signal_data["band"] = active_bands[
                                        0
                                    ]  # Primary band is first one

                                if (
                                    "primary_band" not in signal_data
                                    or signal_data["primary_band"] == "--"
                                ):
                                    signal_data["primary_band"] = active_bands[0]
                        except Exception as e:
                            self.log_message(
                                f"Error parsing band hex: {str(e)}", log_type="detailed"
                            )
            except Exception as e:
                self.log_message(
                    f"Error getting net_mode: {str(e)}", log_type="detailed"
                )
            ...
            if (
                ...
                try:
                    ...
                    if (
                        "CurrentBand" in status_info
                        and status_info["CurrentBand"] != "--"
                    ):
                    ...
                    for field in band_info_fields:
                        if field in status_info and status_info[field] != "--":
                            if (
                                "bands" not in signal_data
                                or signal_data["bands"] == "--"
                            ):
                                signal_data["bands"] = status_info[field]
                except Exception as e:
                    self.log_message(
                        f"Error getting additional monitoring status: {str(e)}",
                        log_type="detailed",
                    )
            ...
            if (
                ...
                try:
                    ...
                    self.log_message(
                        f"Error getting PLMN info: {str(e)}", log_type="detailed"
                    )
                ...
                try:
                    ...
                    self.log_message(
                        f"Error in final band detection: {str(e)}", log_type="detailed"
                    )

            # If we have a bands list but no primary band, set it
            if "bands" in signal_data and (
                "band" not in signal_data
                or signal_data["band"] == "--"
                or signal_data["band"] == "LTE"
            ):
            ...
            if "mode" in signal_data and (
                "band" not in signal_data or signal_data["band"] == "--"
            ):
                signal_data["band"] = signal_data["mode"]

        except Exception as e:
            self.log_message(
                f"Error getting network info: {str(e)}", log_type="detailed"
            )
        ...
        try:
            ...
            self.log_message(
                f"Current traffic rates - DL: {current_dl_rate / 1024:.2f} KB/s, UL: {current_ul_rate / 1024:.2f} KB/s",
                log_type="detailed",
            )
            ...
            self.log_message(
                f"Error getting traffic stats: {str(e)}", log_type="detailed"
            )
        ...
        if hasattr(self, "save_debug_data") and self.save_debug_data:
            ...
            debug_file = os.path.join(
                debug_dir, f"processed_signal_data_{timestamp}.json"
            )
            with open(debug_file, "w") as f:
                json.dump(signal_data, f, indent=4, default=str)
            self.log_message(
                f"Saved processed signal data to {debug_file}", log_type="detailed"
            )
...
def fetch_signal_data(self, session, ip, token):
    ...
    if hasattr(session, "device") and session.__class__.__name__ == "Client":
        try:
            ...
            error_str = str(e)
            self.log_message(
                f"Error fetching signal data from API: {error_str}", log_type="both"
            )

            # Check for session timeout
            if (
                "100003" in error_str
                or "No rights" in error_str
                or "needs login" in error_str
            ):
                ...
                if (
                    ...
                    self.log_message(
                        "Session timeout detected in fetch_signal_data. Attempting to reconnect...",
                        log_type="both",
                    )
                    reconnection_successful = self.silent_reconnect(
                        ip, self.username.get(), self.password.get(), True
                    )
                    ...
                    if reconnection_successful:
                        ...
                        try:
                            ...
                            self.log_message(
                                f"Failed to fetch data after reconnection: {str(retry_e)}",
                                log_type="both",
                            )
    ...
    try:
        ...
        headers = {"Cookie": f"SessionID={token}", "__RequestVerificationToken": token}
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    reconnection_successful = self.silent_reconnect(
                        ip, self.username.get(), self.password.get(), False
                    )
        ...
        signal_data["rsrp"] = (
            root.find(".//rsrp").text if root.find(".//rsrp") is not None else "--"
        )
        signal_data["rsrq"] = (
            root.find(".//rsrq").text if root.find(".//rsrq") is not None else "--"
        )
        signal_data["sinr"] = (
            root.find(".//sinr").text if root.find(".//sinr") is not None else "--"
        )
        signal_data["rssi"] = (
            root.find(".//rssi").text if root.find(".//rssi") is not None else "--"
        )
        signal_data["mode"] = (
            root.find(".//mode").text if root.find(".//mode") is not None else "--"
        )
        ...
        if response.ok:
            root = ET.fromstring(response.text)
            signal_data["plmn"] = (
                root.find(".//ShortName").text
                if root.find(".//ShortName") is not None
                else "--"
            )
            ...
            try:
                bands = (
                    root.find(".//bands").text
                    if root.find(".//bands") is not None
                    else "--"
                )
                primary_band = (
                    root.find(".//primary_band").text
                    if root.find(".//primary_band") is not None
                    else "--"
                )
                ...
                self.log_message(
                    f"Detailed band info - All bands: {bands}, Primary: {primary_band}",
                    log_type="detailed",
                )
            except Exception as e:
                self.log_message(
                    f"Failed to parse detailed 5G band info: {str(e)}",
                    log_type="detailed",
                )
        ...
        if hasattr(self, "get_recent_speedtest_results"):
            ...
            if recent_speedtest:
                signal_data["download"] = (
                    f"{recent_speedtest.get('download', '0.00')} Mbps"
                )
        ...
        self.log_message(
            f"Error fetching signal data from web UI: {str(e)}", log_type="both"
        )
        ...
        if (
            ...
            if (
                ...
                self.log_message(
                    "Session timeout detected in fetch_signal_data exception. Attempting to reconnect...",
                    log_type="both",
                )
                reconnection_successful = self.silent_reconnect(
                    ip, self.username.get(), self.password.get(), False
    ...
    if HUAWEI_API_AVAILABLE and isinstance(session, Client):
        try:
            ...
            try:
                ...
                if (
                    "nrdlbandwidth" in signal_info
                    and signal_info["nrdlbandwidth"]
                    and signal_info["nrdlbandwidth"]
                    not in ["--", "0", "0.0", "0MHz", "0.0MHz"]
                ):
                    has_active_5g = True

                if (
                    "nrdlfreq" in signal_info
                    and signal_info["nrdlfreq"]
                    and signal_info["nrdlfreq"] not in ["--", "0", "0.0"]
                ):
                    has_active_5g = True
            except:
            ...
            network_type_text = network_types.get(
                network_type, f"Unknown ({network_type})"
            )
            if not has_active_5g and (
                "5G" in network_type_text or "NR" in network_type_text
            ):
            ...
            return {"status": connection_status, "network_type": network_type_text}
        except Exception as e:
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                "network_type": network_types.get(
                    network_type.text, f"Unknown ({network_type.text})"
                )
...
def apply_band_lock(session, ip, token, selected_bands):
    ...
    if isinstance(session, Client):
        try:
            ...
            band_hex = (
                sum(BAND_MAP.get(num, 0) for num in band_numbers) or 0x3FFFFFFF
            )  # Default to all bands if none selected
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(
                    f"Band lock failed with status code: {response.status_code}"
                )
            return True
        except Exception as e:
...
def run_speedtest(server_id=None):
    ...
    try:
        ...
        return {"success": False, "message": f"Speed test failed: {str(e)}"}
...
def generate_report(results, optimisation_type="basic"):
    ...
    with open(report_path, "w") as f:
        ...
        for band, result in sorted_results:
            ...
            network_type = result.get(
                "network_type", "4G"
            )  # Default to 4G if not specified
        ...
        if best_bands:
            ...
            f.write("---------------\n")
            f.write(
                "Bands are ranked based on a combination of signal quality (RSRP and SINR) metrics.\n"
            )
            f.write(
                "Even if speedtests fail, bands with good signal quality may provide better overall performance.\n"
            )
            f.write(
                "The optimal configuration typically combines bands with complementary characteristics:\n"
            )
            f.write(
                "- Lower bands (B8, B20, B28) provide better coverage and building penetration\n"
            )
            f.write(
                "- Higher bands (B1, B3, B7) provide better speeds in good conditions\n\n"
            )
            ...
            if has_efficiency:
                ...
                for band, result in has_efficiency:
                    ...
                    f.write(
                        f"{band_str}: Download efficiency: {dl_eff}, Upload efficiency: {ul_eff}\n"
                    )
        else:
...
class BandOptimiserApp(tk.Frame):
    def __init__(self, master=None):
        ...
        self.available_bands = {"4G": SUPPORTED_4G_BANDS, "5G": SUPPORTED_5G_BANDS}
    ...
    def load_config(self):
        ...
        try:
            ...
            )  # Load the password directly

            # Load other settings
            self.auto_connect.set(config.get("auto_connect", False))
            self.auto_apply_bands.set(
                config.get("auto_apply_bands", False)
            ...
            self.notify_on_signal_change.set(
                config.get("notify_on_signal_change", True)
            )
    ...
    def create_menu(self):
        ...
        tools_menu.add_command(
            label="Enhanced Optimize", command=self.enhanced_optimise
        )
        ...
        options_menu.add_checkbutton(
            label="Auto-apply bands on connect",
        ...
        options_menu.add_checkbutton(
            label="Notify on signal changes",
        ...
        self.save_debug_var = tk.BooleanVar(
            value=getattr(self, "save_debug_data", False)
        )

        debug_menu.add_checkbutton(
            label="Save Debug Data (JSON Files)",
        ...
        options_menu.add_command(
            label="Support Development (Donate)", command=self.show_donation_dialog
        )
    ...
    def toggle_debug_data(self):
        ...
        self.log_message(
            f"Debug data saving {'enabled' if self.save_debug_data else 'disabled'}",
            log_type="both",
        )
        self.save_config()
    ...
    def create_main_frame(self):
        ...
        connection_frame = ttk.LabelFrame(
            main_frame, text="Router Connection", padding="5"
        )
        connection_frame.pack(fill=tk.X, padx=5, pady=5)

        # Add error status label for displaying errors without popups
        self.error_display = ttk.Label(
            main_frame, text="", foreground="red", wraplength=780
        )
        self.error_display.pack(fill=tk.X, padx=5, pady=5)

        # Create grid for connection form
        ttk.Label(connection_frame, text="Router IP:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5
        )
        ttk.Entry(connection_frame, textvariable=self.router_ip).grid(
        ...
        ttk.Entry(connection_frame, textvariable=self.username).grid(
        ...
        password_entry = ttk.Entry(
            connection_frame, textvariable=self.password, show="*"
        )
        ...
        auto_connect_cb = ttk.Checkbutton(
            options_frame, text="Auto-connect at startup", variable=self.auto_connect
        )
        auto_connect_cb.pack(anchor=tk.W, pady=2)

        auto_apply_cb = ttk.Checkbutton(
            options_frame,
            text="Auto-apply bands on connect",
            variable=self.auto_apply_bands,
        )
        auto_apply_cb.pack(anchor=tk.W, pady=2)

        if HUAWEI_API_AVAILABLE:
            api_lib_cb = ttk.Checkbutton(
                options_frame,
                text="Use Huawei LTE API library",
                variable=self.use_api_lib,
            )
            api_lib_cb.pack(anchor=tk.W, pady=2)

        speedtest_startup_cb = ttk.Checkbutton(
            options_frame,
        ...
        minimize_tray_cb = ttk.Checkbutton(
            options_frame,
        ...
        if TOOLTIPS_AVAILABLE:
            create_tooltip(
                self.connect_button,
                "Connect to your Huawei router using the provided IP address and credentials",
            )
        ...
        for i, (label_text, key, default) in enumerate(signal_labels):
            row, col = divmod(i, 2)
            ttk.Label(signal_frame, text=label_text).grid(
                row=row, column=col * 2, sticky=tk.W, padx=2, pady=1
            )
            self.signal_info[key] = tk.StringVar(value=default)
            ttk.Label(signal_frame, textvariable=self.signal_info[key]).grid(
        ...
        self.refresh_button = ttk.Button(
            refresh_frame, text="Refresh Signal", command=self.refresh_signal
        )
        self.refresh_button.pack(side=tk.LEFT, padx=2)
        create_tooltip(
            self.refresh_button,
            "Refresh signal information to show current band, signal strength, and network type (4G/5G)",
        )
        ...
        bands_frame = ttk.LabelFrame(
            left_col, text="Band Selection & Management", padding="5"
        )
        ...
        create_tooltip(
            select_all_button,
            "Select all available frequency bands (both 4G and 5G) for testing",
        )
        ...
        self.apply_bands_button = ttk.Button(
            band_buttons_frame,
        ...
        create_tooltip(
            self.apply_bands_button,
            "Apply the selected bands to your router - allows only the selected bands to be used",
        )
        ...
        explanation = ttk.Label(
            bands_frame, text="Advanced Options:", font=("", 9, "bold")
        )
        explanation.pack(anchor=tk.W, padx=5, pady=2)

        explanation_text = ttk.Label(
            bands_frame,
            text="Network aggregation allows separate control of upload/download bands.\n"
        ...
        aggregation_frame = ttk.LabelFrame(
            bands_frame, text="Network Aggregation", padding="5"
        )
        ...
        create_tooltip(
            apply_network_button,
            "Apply the network aggregation settings - allows separate control of upload and download bands",
        )

        # Network Mode Quickswitch section
        quickswitch_frame = ttk.LabelFrame(
            bands_frame, text="Network Mode Quickswitch", padding="5"
        )
        ...
        apply_mode_button = ttk.Button(
            mode_frame, text="Apply", command=self.apply_network_mode, width=10
        )
        apply_mode_button.pack(side=tk.LEFT, padx=2, pady=2)
        create_tooltip(
            apply_mode_button,
            "Apply the selected network mode (2G/3G/4G/5G) to your router",
        )
        ...
        self.optimise_button = ttk.Button(
            action_grid, text="Optimise Bands", command=self.optimise, width=18
        )
        self.optimise_button.grid(row=0, column=0, padx=2, pady=2, sticky="ew")
        create_tooltip(
            self.optimise_button,
            "Automatically test all frequency bands for both 4G and 5G connections. Evaluates signal quality metrics (RSRP, SINR) and recommends the best combination based on coverage and reliability.",
        )

        self.enhanced_optimise_button = ttk.Button(
            action_grid,
            text="Enhanced Optimise",
            command=self.enhanced_optimise,
            width=18,
        )
        self.enhanced_optimise_button.grid(row=0, column=1, padx=2, pady=2, sticky="ew")
        create_tooltip(
            self.enhanced_optimise_button,
            "Advanced optimisation that tests all bands with both signal quality AND speed tests. Tests both 4G and 5G, runs actual speed tests for each band, and provides the most accurate recommendations.",
        )

        self.speedtest_button = ttk.Button(
            action_grid, text="Speed Test", command=self.start_speedtest, width=18
        )
        self.speedtest_button.grid(row=1, column=0, padx=2, pady=2, sticky="ew")
        create_tooltip(
            self.speedtest_button,
            "Run a speed test using the current band configuration. Tests download and upload speeds using the nearest server.",
        )
        ...
        standard_log_scrollbar = ttk.Scrollbar(
            standard_log_container, command=self.log_text.yview
        )
        ...
        self.detailed_log_text = tk.Text(
            detailed_log_container, wrap=tk.WORD, height=15
        )
        self.detailed_log_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        detailed_log_scrollbar = ttk.Scrollbar(
            detailed_log_container, command=self.detailed_log_text.yview
        )
                ...
                try:
                    ...
                    if log_type in ["detailed", "both"]:
                        if (
                            ...
                            and self.detailed_log_text is not None
                        ):
                            if replace_last:
                                # Delete the last line
                                self.detailed_log_text.delete("end-1l", "end")
                            self.detailed_log_text.insert(
                                tk.END, timestamped_message + "\n"
                            )
    ...
    def connect(self):
        ...
        threading.Thread(
            target=lambda: self.connect_thread(ip, username, password, use_api),
            daemon=True,
        ).start()
    ...
    def disconnect(self):
        ...
        if hasattr(self, "is_connected") and self.is_connected:
            try:
                ...
                if (
                    hasattr(self, "poll_status_task")
                    and self.poll_status_task is not None
                ):
                ...
                if hasattr(self, "connect_button"):
                    self.connect_button.config(
                        text="Connect", command=self.connect, state=tk.NORMAL
                    )
    ...
    def connect_thread(self, ip, username, password, use_api):
        ...
        try:
            ...
            self.master.after(
                0, lambda: messagebox.showerror("Connection Error", error_message)
            )
            ...
            if result and len(result) >= 3:
                ...
                if session_or_client is not None:
                    ...
                    if (
                        ...
                        and HUAWEI_API_AVAILABLE
                        and hasattr(session_or_client, "device")
                    ):
                        # API client
                        self.client = session_or_client
                        self.session = None
                        self.log_message(
                            f"Connected using Huawei LTE API (client initialized: {self.client is not None})",
                            log_type="both",
                        )
                    else:
                        # Web session
                        ...
                        self.client = None
                        self.log_message(
                            f"Connected using web session (session initialized: {self.session is not None})",
                            log_type="both",
                        )
                    ...
                    self.connect_button.config(
                        text="Disconnect", command=self.disconnect
                    )
                    ...
                    if self.auto_refresh.get():
                        ...
                        self.poll_status()
                        self.log_message(
                            "Auto-refresh enabled. Signal will update every 30 seconds.",
                            log_type="both",
                        )

                    # Scan for available bands
                    threading.Thread(
                        target=self.update_band_selection_ui, daemon=True
                    ).start()
                    ...
                    if self.auto_apply_bands.get():
                        ...
                        selected_bands = [
                            band for band, var in self.band_vars.items() if var.get()
                        ]
                        if selected_bands:
                            self.log_message(
                                f"Auto-applying band selection from config: {', '.join(selected_bands)}",
                                log_type="both",
                            )
                            threading.Thread(
                                target=lambda: self.apply_band_thread(selected_bands),
                                daemon=True,
                            ).start()
                        else:
                            self.log_message(
                                "Auto-apply bands enabled but no bands selected in config",
                                log_type="both",
                            )
                    ...
                    self.log_message(
                        f"❌ Connection failed: {error_message}", log_type="both"
                    )
    ...
    def show_enhanced_optimisation_summary(
        self, results_4g, results_5g, recommended_results, report_path
    ):
        ...
        ttk.Label(
            summary_frame,
            text="Enhanced Optimisation Results",
        ...
        if "4G" in recommended_results and "5G" in recommended_results:
            ...
            if (
                speed_5g > speed_4g * 1.2 and stability_5g >= stability_4g * 0.8
            ):  # 5G needs to be 20% faster and not more than 20% less stable
                best_option = "5G"
                best_speed = speed_5g
        ...
        def apply_4g():
            if "4G" not in recommended_results:
                messagebox.showwarning(
                    "Not Available", "No viable 4G configuration found."
                )
                return

            self.log_message(
                f"🔄 Applying 4G configuration: {', '.join(recommended_results['4G']['bands'])}",
                log_type="both",
            )
                ...
                if success:
                    self.master.after(
                        ...
                        lambda: self.log_message(
                            "✅ 4G configuration applied successfully!", log_type="both"
                        ),
                    )
                    ...
                    for band_num, var in self.band_vars.items():
                        ...
                        self.master.after(
                            0,
                            lambda bn=band_str, v=var: v.set(
                                bn in recommended_results["4G"]["bands"]
                            ),
                        )
                    ...
                    self.master.after(
                        0,
                        lambda: self.log_message(
                            "❌ Failed to apply 4G configuration.", log_type="both"
                        ),
                    )
        ...
        def apply_5g():
            if "5G" not in recommended_results:
                messagebox.showwarning(
                    "Not Available", "No viable 5G configuration found."
                )
                return

            self.log_message(
                f"🔄 Applying 5G configuration: {', '.join(recommended_results['5G']['bands'])}",
                log_type="both",
            )
            ...
            def apply_thread():
                ...
                if success:
                    self.master.after(
                        0,
                        lambda: self.log_message(
                            "✅ 5G configuration applied successfully!", log_type="both"
                        ),
                    )
                    ...
                    for band_num, var in self.band_vars.items():
                        ...
                        self.master.after(
                            0,
                            lambda bn=band_str, v=var: v.set(
                                bn in recommended_results["5G"]["bands"]
                            ),
                        )
                    ...
                    self.master.after(
                        0,
                        lambda: self.log_message(
                            "❌ Failed to apply 5G configuration.", log_type="both"
                        ),
                    )
        ...
        def on_cancel():
            self.log_message(
                "Optimisation cancelled. Restoring original band configuration.",
                log_type="both",
            )
            ...
            if original_band_config:
                ...
                def restore_thread():
                    self.log_message(
                        f"Restoring bands: {', '.join(original_band_config)}",
                        log_type="detailed",
                    )
                    ...
                    if success:
                        self.master.after(
                            0,
                            lambda: self.log_message(
                                "✅ Original configuration restored", log_type="both"
                            ),
                        )
                        ...
                        self.master.after(
                            ...
                            lambda m=msg: self.log_message(
                                f"❌ Failed to restore configuration: {m}",
                                log_type="both",
                            ),
                        )
                ...
                self.log_message(
                    "No previous configuration to restore", log_type="detailed"
                )
        ...
        if "4G" in recommended_results:
            ttk.Button(
                button_frame, text="Apply 4G Configuration", command=apply_4g
            ).pack(side=tk.LEFT, padx=5)
        if "5G" in recommended_results:
            ttk.Button(
                button_frame, text="Apply 5G Configuration", command=apply_5g
            ).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="View Report", command=view_report).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def update_band_selection_ui(self):
        ...
        self.log_message(
            f"Processed 4G bands: {', '.join([f'B{b}' for b in processed_4g_bands])}",
            log_type="both",
        )
        self.log_message(
            f"Processed 5G bands: {', '.join([f'n{b}' for b in processed_5g_bands])}",
            log_type="both",
        )
        ...
        if hasattr(self, "band_section_4g") and hasattr(self, "band_section_5g"):
            ...
            for band_num in sorted_4g_bands:
                ...
                if (
                    current_active_band
                    and band_name.upper() == current_active_band.upper()
                ):
                    is_active = True
            ...
            self.log_message(
                "Band selection updated with available bands", log_type="both"
            )
            ...
            self.log_message(
                "Could not find band selection UI - please restart the application",
                log_type="both",
            )
    ...
    def toggle_auto_refresh(self):
        ...
        if self.auto_refresh.get():
            ...
            if self.is_connected:
                ...
                self.log_message(
                    "Auto-refresh will begin after connecting to router",
                    log_type="both",
                )
        else:
    ...
    def refresh_signal(self):
        ...
        if not self.session and not self.client:
            self.log_message(
                "Not connected to router. Please connect first.", log_type="both"
            )
            return
        ...
        if hasattr(self, "manual_selection_refresh_count"):
            ...
            if self.manual_selection_refresh_count >= 5:
                ...
                self.log_message(
                    "Manual band selections expired, syncing with router state",
                    log_type="detailed",
                )
    ...
    def silent_reconnect(self, ip, username, password, use_api=True):
        ...
        self.log_message(
            f"Silent reconnect attempt to {ip}... (API mode: {use_api})",
            log_type="detailed",
        )
        ...
        try:
            ...
            if not hasattr(self, "poll_status_task") or not self.poll_status_task:
                self.poll_status()
            ...
            self.log_message(
                f"Silent reconnect successful to {ip} using {self.connection_type}",
                log_type="both",
            )
            return True
            ...
            if self.reconnection_attempts < 5:
                self.log_message(
                    f"Scheduling retry #{self.reconnection_attempts + 1} in 10 seconds...",
                    log_type="detailed",
                )
                self.master.after(
                    10000,
                    lambda: self.silent_reconnect(ip, username, password, use_api),
                )
            else:
                self.log_message(
    ...
    def session_keepalive(self):
        ...
        try:
            ...
            if (
                hasattr(self, "last_session_activity")
                and (current_time - self.last_session_activity) > 120
            ):
                self.log_message(
                    "Sending session keepalive request", log_type="detailed"
                )
                ...
                if hasattr(self, "client") and self.client:
                    ...
                    try:
                        ...
                        self.log_message(
                            f"Keepalive successful (API) - Device: {device_name}",
                            log_type="detailed",
                        )
                    except Exception as e:
                        error_str = str(e)
                        if (
                            ...
                            self.log_message(
                                "Session expired during keepalive check. Reconnecting...",
                                log_type="both",
                            )
                            self.session_timeout_detected = True
                            ...
                            if hasattr(self, "username") and hasattr(self, "password"):
                                ...
                                if reconnect_result:
                                    self.log_message(
                                        "Reconnected successfully during keepalive",
                                        log_type="both",
                                    )
                                else:
                                    self.log_message(
                                        "Failed to reconnect during keepalive",
                                        log_type="both",
                                    )
                        else:
                            self.log_message(
                                f"Keepalive request failed: {error_str}",
                                log_type="detailed",
                            )
                    ...
                    try:
                        ...
                        response = self.session.get(
                            f"http://{ip}/api/monitoring/status", headers=headers
                        )
                        if response.ok:
                            self.last_session_activity = time.time()
                            self.log_message(
                                "Keepalive successful (Web)", log_type="detailed"
                            )
                            ...
                            self.log_message(
                                f"Keepalive request returned status {response.status_code}: {response.text[:100]}",
                                log_type="detailed",
                            )
                    except Exception as e:
                        self.log_message(
                            f"Keepalive request failed: {str(e)}", log_type="detailed"
                        )
                        # Check if this is a connection error and attempt reconnection
                        if "Connection" in str(e) or "Timeout" in str(e):
                            self.log_message(
                                "Connection error during keepalive. Attempting reconnect...",
                                log_type="both",
                            )
                            reconnect_result = self.silent_reconnect(
                                self.router_ip.get(),
                            ...
                            if reconnect_result:
                                self.log_message(
                                    "Reconnected successfully after connection error",
                                    log_type="both",
                                )
                else:
                    # No valid client or session
                    self.log_message(
                        "No valid connection found during keepalive. Attempting reconnect...",
                        log_type="both",
                    )
                    reconnect_result = self.silent_reconnect(
                        self.router_ip.get(),
                        ...
                        self.password.get(),
                        True,
                    )  # Default to API mode for reconnect
                    if reconnect_result:
                        self.log_message(
                            "Reconnected successfully after missing connection",
                            log_type="both",
                        )
            else:
                ...
                if hasattr(self, "client") and self.client:
                    try:
                        ...
                        if (
                            ...
                            # Session already expired, attempt reconnection immediately
                            self.log_message(
                                "Session expired during light check. Reconnecting...",
                                log_type="detailed",
                            )
                            self.session_timeout_detected = True
                            # Attempt reconnection
                    ...
                    try:
                        ...
                        response = self.session.get(
                            f"http://{ip}/api/device/basic_information", headers=headers
                        )
                        if not response.ok and "100003" in response.text:
                            # Session expired, reconnect
                            self.log_message(
                                "Session expired during light web check. Reconnecting...",
                                log_type="detailed",
                            )
                            self.session_timeout_detected = True
                            self.silent_reconnect(
    ...
    def poll_status(self):
        ...
        try:
            ...
            if not self.auto_refresh.get():
                self.log_message(
                    "Polling stopped - auto-refresh disabled", log_type="detailed"
                )
                self.poll_status_task = None
                return
            ...
            if (
                hasattr(self, "session_timeout_detected")
                and self.session_timeout_detected
            ):
                self.log_message(
                    "Session timeout detected. Attempting auto-reconnect...",
                    log_type="both",
                )
            ...
            if hasattr(self, "poll_failure_count") and self.poll_failure_count >= 5:
                ...
                self.master.after(
                    0, lambda: self.error_display.config(text=error_message)
                )
            ...
            try:
                ...
                if (
                    "No rights" in error_str
                    or "needs login" in error_str
                    or "100003" in error_str
                    or "login required" in error_str
                ):
                    self.log_message(
                        "Session timeout detected in polling, will attempt reconnect",
                        log_type="detailed",
                    )
                    self.session_timeout_detected = True
                ...
                self.poll_failure_count += 1
                self.log_message(
                    f"Error in auto-refresh ({self.poll_failure_count}/5): {str(e)}",
                    log_type="both",
                )
            ...
            if self.is_connected and self.auto_refresh.get():
                try:
                    ...
                    interval = (
                        ...
                        if hasattr(self, "session_timeout_detected")
                        and self.session_timeout_detected
                        else self.signal_update_interval
                    )
                    self.poll_status_task = self.master.after(
                        interval, self.poll_status
                    )
                    self.log_message(
                        f"Scheduled next poll in {interval / 1000} seconds",
                        log_type="detailed",
                    )
                except Exception as e:
                    self.log_message(
                        f"Error scheduling next poll: {str(e)}", log_type="detailed"
                    )
            else:
                self.poll_status_task = None
    ...
    def start_speedtest(self):
        ...
        if not hasattr(self, "is_connected") or not self.is_connected:
            self.log_message(
                "Cannot run speedtest: Not connected to router", log_type="both"
            )
            messagebox.showwarning(
                "Not Connected", "Please connect to the router first."
            )
            return
        ...
        self.log_message(
            "Starting speed test. This may take a minute...", log_type="both"
        )
        ...
        def speedtest_thread():
            try:
                ...
                if result["success"]:
                    ...
                    self.master.after(
                        0, lambda: self.log_message(success_message, log_type="both")
                    )
                    ...
                    if (
                        hasattr(self, "signal_info")
                        and "DOWNLOAD" in self.signal_info
                        and "UPLOAD" in self.signal_info
                    ):
                        self.master.after(
                            0,
                            lambda: self.signal_info["DOWNLOAD"].set(
                                f"{download:.2f} Mbps"
                            ),
                        )
                        self.master.after(
                            0,
                            lambda: self.signal_info["UPLOAD"].set(
                                f"{upload:.2f} Mbps"
                            ),
                        )
                else:
                    error_message = (
                        f"❌ Speed test failed: {result.get('error', 'Unknown error')}"
                    )
                    self.master.after(
                        0, lambda: self.log_message(error_message, log_type="both")
                    )
                    self.master.after(
                        0,
                        lambda: messagebox.showerror(
                            "Speed Test Failed", error_message
                        ),
                    )
                ...
                self.master.after(
                    0, lambda: self.log_message(f"❌ {error_message}", log_type="both")
                )
                self.master.after(
                    0, lambda: messagebox.showerror("Speed Test Error", error_message)
                )
                ...
                if hasattr(self, "speedtest_button"):
                    self.master.after(
                        0, lambda: self.speedtest_button.config(state=tk.NORMAL)
                    )
    ...
    def apply_network_config(self):
        ...
        if not selected_upload_bands and not selected_download_bands:
            messagebox.showwarning(
                "Selection Empty",
                "Please select at least one band for upload or download.",
            )
            return

        # Log the selected bands
        self.log_message(
            f"Applying network aggregation settings - Upload: {', '.join(selected_upload_bands)}, Download: {', '.join(selected_download_bands)}",
            log_type="both",
        )
        ...
        def apply_thread():
            try:
                ...
                upload_band_nums = [
                    int(band.replace("B", "")) for band in selected_upload_bands
                ]
                download_band_nums = [
                    int(band.replace("B", "")) for band in selected_download_bands
                ]
                    ...
                    if hasattr(session, "net"):
                        ...
                        if upload_response == "OK" or (
                            ...
                            def report_failure_upload():
                                self.log_message(
                                    "❌ Failed to apply upload bands", log_type="both"
                                )
                        ...
                        if response.status_code != 200:
                            ...
                            def report_failure_token():
                                self.log_message(
                                    "❌ Failed to get CSRF token", log_type="both"
                                )
                        ...
                        net_mode_url = (
                            f"http://{self.router_ip.get()}{NET_MODE_ENDPOINT}"
                        )
                        ...
                        if response.status_code != 200:
                            ...
                            def report_failure_mode():
                                self.log_message(
                                    "❌ Failed to get network mode", log_type="both"
                                )
                ...
                if selected_download_bands:
                    ...
                    if hasattr(session, "net"):
                        ...
                        if download_response == "OK" or (
                            isinstance(download_response, dict)
                            and download_response.get("result") == "success"
                        ):
                            # Use a regular function instead of a lambda
                            def report_success_download():
                                self.log_message(
                                    "✅ Download bands applied successfully",
                                    log_type="both",
                                )
                            ...
                            def report_failure_download():
                                self.log_message(
                                    "❌ Failed to apply download bands", log_type="both"
                                )
                ...
                def report_error():
                    self.log_message(
                        f"❌ Error applying network configuration: {error_msg}",
                        log_type="both",
                    )
    ...
    def toggle_all_bands(self, state, band_type="all"):
        ...
        try:
            ...
            for band in self.available_bands.get("4G", []):
                if isinstance(band, str) and band.startswith("B"):
                    processed_4g_bands.append(band)
                elif isinstance(band, int) or (
                    isinstance(band, str) and band.isdigit()
                ):
            ...
            self.log_message(
                f"{'Selected' if state else 'Cleared'} all {band_type} bands",
                log_type="both",
            )
    ...
    def apply_band_selection(self):
        ...
        if not hasattr(self, "is_connected") or not self.is_connected:
            self.log_message(
                "⚠️ Not connected to router. Cannot apply band selection.",
                log_type="both",
            )
            return
        ...
        try:
            ...
            if not selected_bands:
                self.log_message(
                    "⚠️ No bands selected. Please select at least one band.",
                    log_type="both",
                )
                if hasattr(self, "apply_bands_button"):
            ...
            self.log_message(
                f"🔄 Applying band selection: {', '.join(selected_bands)}",
                log_type="both",
            )

            # Start the apply operation in a background thread
            threading.Thread(
                target=lambda: self.apply_band_thread(selected_bands), daemon=True
            ).start()

        except Exception as e:
            self.log_message(
                f"❌ Error preparing band selection: {str(e)}", log_type="both"
            )
            if hasattr(self, "apply_bands_button"):
                self.apply_bands_button.config(state=tk.NORMAL)
    ...
    def apply_band_thread(self, selected_bands):
        ...
        try:
            ...
            self.master.after(
                0,
                lambda: self.log_message(
                    "✅ Band selection applied successfully!", log_type="both"
                ),
            )
            ...
            if hasattr(self, "apply_bands_button"):
                self.master.after(
                    0, lambda: self.apply_bands_button.config(state=tk.NORMAL)
                )
    ...
    def optimise(self):
        ...
        self.log_message(
            "This process will test all available bands and recommend the best combination.",
            log_type="standard",
        )
        ...
        self.log_message(
            f"Current band config saved: {', '.join(original_band_config) if original_band_config else 'No bands'}",
            log_type="detailed",
        )
        ...
        def optimise_thread():
            try:
                ...
                bands_to_test = (
                    self.available_bands["4G"]
                    if hasattr(self, "available_bands")
                    else []
                )
                ...
                for band in bands_to_test:
                    ...
                    try:
                        ...
                        if "Band not supported by this device" in str(e):
                            self.log_message(
                                f"⚠️ Band {band} is not supported by this device",
                                log_type="both",
                            )
                            results[band] = {
                                "score": 0,
                    ...
                    try:
                        if hasattr(self, "client") and self.client:
                            # Using API client
                            signal_data = fetch_signal_data_api(
                                self, self.client, self.router_ip.get()
                            )
                        else:
                            # Using web client
                            signal_data = fetch_signal_data(
                                self, self.session, self.router_ip.get(), self.token
                            )

                        # Check if we got valid signal data
                        if not signal_data or not isinstance(signal_data, dict):
                            self.log_message(
                                f"❌ Failed to get signal data for band {band}",
                                log_type="both",
                            )
                            results[band] = {
                                "score": 0,
                        ...
                        self.log_message(
                            f"❌ Error getting signal data: {str(e)}", log_type="both"
                        )
                        results[band] = {
                            "score": 0,
                            "rsrp": None,
                    ...
                    # Store results
                    results[band] = {"score": score, "rsrp": rsrp, "sinr": sinr}

                    self.log_message(
                        f"Band {band}: RSRP={rsrp} dBm, SINR={sinr} dB, Score={score:.1f}",
                        log_type="detailed",
                    )

                    successful_tests += 1
                    self.log_message(
                        f"✅ Tested {successful_tests}/{len(bands_to_test)} bands",
                        log_type="standard",
                    )
                ...
                top_bands = [band for band, data in sorted_bands if data["score"] > 0][
                    :3
                ]

                if not top_bands:
                    error_message = (
                        "⚠️ No usable bands found. Try again or check connection."
                    )
                    self.log_message(error_message, log_type="both")
                    self.master.after(
                        0,
                ...
                self.master.after(
                    0,
                    lambda: self.show_enhanced_optimisation_summary(
                        top_bands, results, report_path
                    ),
                )
                ...
                self.log_message(error_message, log_type="both")
                self.master.after(
                    0, lambda: messagebox.showerror("Optimization Error", error_message)
                )
                ...
                try:
                    ...
                    if original_band_config:
                        self.log_message(
                            f"Attempting to restore original bands: {', '.join(original_band_config)}",
                            log_type="both",
                        )
                        apply_band_lock(
                            self.session or self.client,
                    ...
                    self.master.after(
                        0,
                        lambda: messagebox.showerror(
                            "Restoration Error", restore_error_message
                        ),
                    )
            finally:
                ...
                if hasattr(self, "optimise_button"):
                    self.master.after(
                        0, lambda: self.optimise_button.config(state=tk.NORMAL)
                    )
                if hasattr(self, "enhanced_optimise_button"):
                    self.master.after(
                        0, lambda: self.enhanced_optimise_button.config(state=tk.NORMAL)
                    )
                if hasattr(self, "speedtest_button"):
                    self.master.after(
                        0, lambda: self.speedtest_button.config(state=tk.NORMAL)
    ...
    def enhanced_optimise(self):
        ...
        if not hasattr(self, "is_connected") or not self.is_connected:
            self.log_message(
                "⚠️ Not connected. Cannot run enhanced optimisation.", log_type="both"
            )
            return
        ...
        threading.Thread(
            target=lambda: self.enhanced_optimise_thread(original_band_config),
            daemon=True,
        ).start()
    ...
    def enhanced_optimise_thread(self, original_band_config):
        try:
            ...
            for band in self.available_bands["4G"]:
                ...
                while not success and retry_count <= max_retries:
                    try:
                        ...
                        if (
                            "wrong session token" in error_str
                            or "125003" in error_str
                            or "100003" in error_str
                            or "needs login" in error_str
                            or "no rights" in error_str
                            ...
                            retry_count += 1
                            self.log_message(
                                f"Session token error while applying band {band}. Reconnecting... (Attempt {retry_count})",
                                log_type="both",
                            )
                            ...
                            if reconnect_success:
                                self.log_message(
                                    "Reconnected successfully, retrying band application",
                                    log_type="both",
                                )
                                time.sleep(2)  # Small delay before retry
                            else:
                                raise Exception(
                                    f"Failed to reconnect after session token error"
                                )
                        else:
                    ...
                    if not success:
                        self.log_message(
                            f"Failed to apply band {band} after {max_retries} attempts, skipping",
                            log_type="both",
                        )
                        continue
                    ...
                    while signal_data is None and retry_count <= max_retries:
                        try:
                            ...
                            if (
                                ...
                                retry_count += 1
                                self.log_message(
                                    f"Session token error while fetching signal data. Reconnecting... (Attempt {retry_count})",
                                    log_type="both",
                                )
                                ...
                                if reconnect_success:
                                    self.log_message(
                                        "Reconnected successfully, retrying signal data fetch",
                                        log_type="both",
                                    )
                                    time.sleep(2)  # Small delay before retry
                                else:
                    ...
                    if not signal_data:
                        self.log_message(
                            f"No signal data for band {band}, skipping", log_type="both"
                        )
                        continue
                    ...
                    self.log_message(
                        f"Band {band}: RSRP={rsrp_float} dBm, SINR={sinr_float} dB, Score={final_score:.1f}",
                        log_type="detailed",
                    )
            ...
            if results_4g:
                sorted_4g = sorted(
                    results_4g.items(), key=lambda x: x[1]["score"], reverse=True
                )
                best_4g_band = sorted_4g[0][0] if sorted_4g else None

                if best_4g_band:
                    self.recommended_results["4G"]["bands"] = [best_4g_band]
                    self.recommended_results["4G"]["score"] = results_4g[best_4g_band][
                        "score"
                    ]
            ...
            if results_5g:
                sorted_5g = sorted(
                    results_5g.items(), key=lambda x: x[1]["score"], reverse=True
                )
                best_5g_band = sorted_5g[0][0] if sorted_5g else None

                if best_5g_band:
                    self.recommended_results["5G"]["bands"] = [best_5g_band]
                    self.recommended_results["5G"]["score"] = results_5g[best_5g_band][
                        "score"
                    ]
            ...
            while not restore_success and retry_count <= max_retries:
                try:
                    self.log_message(
                        "Restoring original band configuration...", log_type="both"
                    )
                    apply_band_lock(
                        self.session or self.client,
                        self.router_ip.get(),
                    ...
                    if (
                        ...
                        retry_count += 1
                        self.log_message(
                            f"Session token error while restoring band configuration. Reconnecting... (Attempt {retry_count})",
                            log_type="both",
                        )
                        ...
                        if reconnect_success:
                            self.log_message(
                                "Reconnected successfully, retrying band restoration",
                                log_type="both",
                            )
                            time.sleep(2)  # Small delay before retry
                        else:
                            self.log_message(
                                "Failed to reconnect after session token error during band restoration",
                                log_type="both",
                            )
                            break
                    else:
                        # Other error, not retry-able
                        self.log_message(
                            f"Error restoring band configuration: {str(e)}",
                            log_type="both",
                        )
                        break
            ...
            try:
                ...
                restore_error_message = (
                    f"Failed to restore original bands: {str(restore_error)}"
                )
                self.log_message(restore_error_message, log_type="both")
                self.master.after(
                    0,
    ...
    def update_signal_ui(self, signal_data):
        ...
        self.log_message(
            f"Raw signal data received: {signal_data}", log_type="detailed"
        )
        ...
        for data_key, ui_key in key_mapping.items():
            if data_key in signal_data and ui_key in self.signal_info:
                ...
                if data_key == "mode" and signal_data[data_key]:
                    ...
                    if (
                        "nrdlbandwidth" in signal_data
                        and signal_data["nrdlbandwidth"]
                        and signal_data["nrdlbandwidth"]
                        not in ["--", "0", "0.0", "0MHz", "0.0MHz"]
                    ):
                        has_active_5g = True
                        self.log_message(
                            f"Detected active 5G NR with bandwidth: {signal_data['nrdlbandwidth']}",
                            log_type="detailed",
                        )

                    # Also check for 5G NR frequencies
                    if (
                        "nrdlfreq" in signal_data
                        and signal_data["nrdlfreq"]
                        and signal_data["nrdlfreq"] not in ["--", "0", "0.0"]
                    ):
                        has_active_5g = True
                        self.log_message(
                            f"Detected active 5G NR with frequency: {signal_data['nrdlfreq']}",
                            log_type="detailed",
                        )
                    ...
                    if mode_val == "101" or mode_val == "21":
                        ...
                        if has_active_5g or has_5g_bands:
                            ...
                            self.log_message(
                                "Confirmed 5G NSA with active 5G components",
                                log_type="detailed",
                            )
                        else:
                            display_value = "4G"
                            self.log_message(
                                "Mode code indicates 5G NSA but no active 5G detected, showing 4G",
                                log_type="detailed",
                            )
                        ...
                        if has_active_5g or has_5g_bands:
                            ...
                            display_value = "4G"
                            self.log_message(
                                "Mode code indicates 5G SA but no active 5G detected, showing 4G",
                                log_type="detailed",
                            )
                        ...
                        if has_active_5g or has_5g_bands:
                            ...
                            display_value = "4G"
                            self.log_message(
                                "Mode code indicates 5G but no active 5G detected, showing 4G",
                                log_type="detailed",
                            )
                        ...
                        if has_lte_bands:
                            display_value = "5G NSA"
                            self.log_message(
                                f"Detected active 5G with 4G bands despite mode code {mode_val}, showing 5G NSA",
                                log_type="detailed",
                            )
                        else:
                            display_value = "5G"
                            self.log_message(
                                f"Detected active 5G despite mode code {mode_val}, showing 5G",
                                log_type="detailed",
                            )
                    else:
                    ...
                    self.log_message(
                        f"Network mode detected: {mode_val} → {display_value}",
                        log_type="detailed",
                    )
                ...
                # Special handling for band to show all bands when available
                    ...
                    if display_value != "N/A" and display_value != "--":
                        ...
                        if "bands" in signal_data and signal_data["bands"]:
                            ...
                            primary_band = signal_data.get(
                                "primary_band", signal_data["band"]
                            )
            ...
            # Special case for PRIMARY_BAND if not in the data
                ...
                ui_key == "PRIMARY_BAND"
                and "band" in signal_data
                and "primary_band" not in signal_data
            ):
        ...
        if (
            "rsrp" in signal_data
            and signal_data["rsrp"] is not None
            and signal_data["rsrp"] != "--"
        ):
            try:
                ...
                self.log_message(
                    f"Could not parse RSRP value: {signal_data['rsrp']} - {str(e)}",
                    log_type="detailed",
                )
            ...
            for widget in self.master.winfo_children():
                if isinstance(widget, tk.Frame):
                    for inner_widget in widget.winfo_children():
                        if (
                            isinstance(inner_widget, ttk.LabelFrame)
                            and inner_widget.winfo_children()
                        ):
                            for label_widget in inner_widget.winfo_children():
                                if (
                                    hasattr(label_widget, "cget")
                                    and label_widget.cget("textvariable")
                                    == self.signal_info["RSRP"]
                                ):
                                    label_widget.config(foreground=color)
                                    return
        except Exception as e:
            self.log_message(
                f"Error updating RSRP color: {str(e)}", log_type="detailed"
            )
    ...
    def select_active_band(self, active_band):
        ...
        if isinstance(active_band, str):
            ...
            if "," in active_band:
                ...
                self.log_message(
                    f"Selecting multiple active bands: {active_bands}",
                    log_type="detailed",
                )
            else:
                # Single band
                ...
                self.log_message(
                    f"Selecting single active band: {active_band}", log_type="detailed"
                )
        else:
            # Not a valid band string
            self.log_message(
                f"Invalid active band format: {active_band}", log_type="detailed"
            )
            return
        ...
        for band in active_bands:
            ...
            if isinstance(band, str):
                ...
                if (
                    not is_5g
                    and not band.upper().startswith("B")
                    and band.replace("B", "").isdigit()
                ):
            ...
            bands_to_try = [
                ...
                clean_band.replace("B", "")
                .replace("b", "")
                .replace("N", "")
                .replace("n", ""),  # e.g., "7" or "78"
            ]
            ...
            for band_to_try in bands_to_try:
                if band_to_try in self.band_vars:
                    ...
                    self.log_message(
                        f"Selected active band: {band_to_try}", log_type="detailed"
                    )
                    found = True
                    break
            ...
            if not found:
                self.log_message(
                    f"Could not find checkbox for band: {band} (not in available bands)",
                    log_type="detailed",
                )
        ...
        if not any(self.band_vars[band].get() for band in self.band_vars):
            ...
            if available_bands:
                self.log_message(
                    f"Available band checkboxes: {available_bands}", log_type="detailed"
                )
    ...
    def check_signal_changes(self, signal_data):
        ...
        if "band" in signal_data and "band" in self.last_signal:
            if signal_data["band"] != self.last_signal["band"]:
                changes.append(
                    f"Band: {self.last_signal['band']} → {signal_data['band']}"
                )
        ...
        if "bands" in signal_data and "bands" in self.last_signal:
            if signal_data["bands"] != self.last_signal["bands"]:
                changes.append(
                    f"All Bands: {self.last_signal['bands']} → {signal_data['bands']}"
                )
        ...
        if "primary_band" in signal_data and "primary_band" in self.last_signal:
            if signal_data["primary_band"] != self.last_signal["primary_band"]:
                changes.append(
                    f"Primary Band: {self.last_signal['primary_band']} → {signal_data['primary_band']}"
                )
        ...
        if changes:
            ...
            if NOTIFICATIONS_AVAILABLE:
                try:
                    ...
                    notifier.show_toast(
                        "Band Change Detected", message, duration=5, threaded=True
                    )
                except Exception as e:
                    self.log_message(
                        f"Error showing notification: {str(e)}", log_type="detailed"
                    )
    ...
    def store_credentials(self, router_ip, username, password, use_api):
        ...
        self.log_message(
            "Stored credentials for auto-reconnection", log_type="detailed"
        )
        ...
        try:
            ...
            if not router_ip:
                messagebox.showerror(
                    "Connection Error", "Please enter router IP address"
                )
                return

            if not username or not password:
                messagebox.showerror(
                    "Connection Error", "Please enter username and password"
                )
                return
            ...
            self.log_message(
                f"Connecting to {router_ip} using {'API' if use_api else 'Web'} mode...",
                log_type="both",
            )
            ...
            if not login_result or not login_result[0]:
                ...
                messagebox.showerror(
                    "Connection Error", f"Failed to connect to router: {error_msg}"
                )
                return
            ...
            self.log_message(
                f"Successfully connected to {router_ip} via {self.connection_type}",
                log_type="both",
            )
            ...
            messagebox.showerror(
                "Connection Error", f"Error connecting to router: {str(e)}"
            )
            self.is_connected = False
    ...
    def auto_reconnect(self):
        ...
        if not hasattr(self, "stored_ip") or not self.stored_ip:
            self.log_message(
                "No stored credentials for auto-reconnection", log_type="both"
            )
            return False

        self.log_message(
            "Auto-reconnecting with stored credentials...", log_type="both"
        )
        try:
    ...
    def update_aggregation_ui(self, signal_data):
        ...
        try:
            ...
            manually_set = hasattr(self, "selected_upload_bands") and hasattr(
                self, "selected_download_bands"
            )

            if manually_set and (
                self.selected_upload_bands or self.selected_download_bands
            ):
                # Use manually selected bands
                self.log_message(
                    "Using manually selected bands for aggregation UI",
                    log_type="detailed",
                )
                ...
                upload_str = (
                    ", ".join(self.selected_upload_bands)
                    if self.selected_upload_bands
                    else "None"
                )
                download_str = (
                    ", ".join(self.selected_download_bands)
                    if self.selected_download_bands
                    else "None"
                )
                self.log_message(
                    f"Detected band aggregation - Upload: {upload_str}, Download: {download_str}",
                    log_type="both",
                )
            ...
            download_bands = signal_data.get(
                "active_band", ""
            )  # Download bands are in the active_band field
            ...
            if (not upload_bands or not download_bands) and all_bands:
                self.log_message(
                    f"Using all_bands as fallback for download bands: {all_bands}",
                    log_type="detailed",
                )
                ...
                if "primary_band" in signal_data and signal_data["primary_band"]:
                    ...
                    self.log_message(
                        f"Using primary_band as fallback for upload band: {upload_bands}",
                        log_type="detailed",
                    )
            ...
            if download_bands:
                self.log_message(
                    f"Processing download bands: {download_bands}", log_type="detailed"
                )
                for band_num in [1, 3, 7, 8]:  # The bands we have UI for
                    band_name = f"B{band_num}"
                    # Check various formats that might indicate this band is active
                    if any(
                        pattern in download_bands
                        for pattern in [
                            f"B{band_num}",
                            f"+{band_num}",
                            f"LTE B{band_num}",
            ...
            if upload_bands_list or download_bands_list:
                self.log_message(
                    f"Detected band aggregation - Upload: {', '.join(upload_bands_list)}, Download: {', '.join(download_bands_list)}",
                    log_type="both",
                )
            else:
                self.log_message(
                    "No bands detected for aggregation UI", log_type="detailed"
                )

        except Exception as e:
            self.log_message(
                f"Error updating aggregation UI: {str(e)}", log_type="detailed"
            )
    ...
    def refresh_thread(self):
        ...
        try:
            ...
            if not hasattr(self, "client") or self.client is None:
                self.log_message(
                    "Client is None, attempting silent reconnect", log_type="detailed"
                )
                ...
                if not silent_reconnect_success:
                    self.log_message(
                        "Silent reconnect failed, cannot refresh signal",
                        log_type="both",
                    )
            ...
            self.log_message(
                "Attempting to fetch signal data with current session",
                log_type="detailed",
            )

            # Fetch signal data using the standalone function
            signal_data = fetch_signal_data(
                self, self.client, self.router_ip.get(), getattr(self, "token", None)
            )

            # If we got an empty result or session timeout was detected, try to reconnect
            if (
                not signal_data
                or hasattr(self, "session_timeout_detected")
                and self.session_timeout_detected
            ):
                self.log_message(
                    "Session timeout detected or empty signal data, attempting to reconnect",
                    log_type="detailed",
                )
                # Reset the flag
                if hasattr(self, "session_timeout_detected"):
        ...
        try:
            ...
            if hasattr(self, "poll_status_task") and self.poll_status_task:
                try:
                    ...
                    self.log_message(
                        f"Error cancelling poll task: {str(e)}", log_type="detailed"
                    )
    ...
    def save_config(self):
        ...
        try:
            config = {
                ...
                "selected_bands": [
                    band_name for band_name, var in self.band_vars.items() if var.get()
                ],
            }
    ...
    def show_donation_dialog(self):
        ...
        try:
            ...
            # Add content
            ttk.Label(dialog, text="Support Development", font=("", 12, "bold")).pack(
                pady=10
            )

            message = ttk.Label(
                dialog, text=donation_text, wraplength=380, justify=tk.CENTER
            )
            ...
            y = (
                self.master.winfo_y()
                + (self.master.winfo_height() - dialog.winfo_height()) // 2
            )
            dialog.geometry(f"+{max(0, x)}+{max(0, y)}")

        except Exception as e:
            self.log_message(
                f"Error showing donation dialog: {str(e)}", log_type="both"
            )
    ...
    def setup_tray_icon(self):
        ...
        try:
            ...
            self.tray_icon = pystray.Icon(
                "Huawei Band Scanner", icon_image, "Huawei Band Scanner", menu
            )
            ...
            self.log_message(
                f"Failed to setup system tray icon: {str(e)}", log_type="both"
            )
    ...
    def _run_tray_icon(self):
        ...
        try:
            ...
            if any(
                err in error_msg
                for err in ["WPARAM is simple", "WNDPROC return value", "LRESULT"]
            ):
                # Silently ignore these specific Windows-related errors
                pass
            else:
...
def scan_available_bands(session, ip, token):
    ...
    if HUAWEI_API_AVAILABLE and isinstance(session, Client):
        try:
            ...
            return {"4G": SUPPORTED_4G_BANDS, "5G": SUPPORTED_5G_BANDS}
